...
driver.get("https://www.ebay.com/str/Crash-Parts-Fast")
...
for sku in sku_list:
    # search button
    # search_bar = driver.find_element_by_name("_bkw")
    # search_bar.clear()
    # search_bar.send_keys(sku)
    # search_button = driver.find_element(By.CSS_SELECTOR,"#keywordSearch.keyword-search.ks form.ks__form button.ks__form__submit").click()

    for i in range(1, 6):
        # search page link
        new_page_link = f"https://www.ebay.com/str/Crash-Parts-Fast?_bkw={sku[0]}"
        driver.get(new_page_link)
        try:
            if i == 2:
                new_page_link = f"https://www.ebay.com/str/Crash-Parts-Fast?_bkw={sku[0]}&_pgn=2"
                driver.get(new_page_link)
            elif i == 3:
                new_page_link = f"https://www.ebay.com/str/Crash-Parts-Fast?_bkw={sku[0]}&_pgn=3"
                driver.get(new_page_link)
            elif i == 4:
                new_page_link = f"https://www.ebay.com/str/Crash-Parts-Fast?_bkw={sku[0]}&_pgn=4"
                driver.get(new_page_link)
            elif i == 5:
                new_page_link = f"https://www.ebay.com/str/Crash-Parts-Fast?_bkw={sku[0]}&_pgn=5&rt=nc"
                driver.get(new_page_link)
        except:
            pass
        ...
        for product_table in product_tables:
            ...
            try:
                ...
                print(p_title)
                parts_from_title = p_title.split(" ")
                p_r_t = parts_from_title[3]
            except:
                p_title  = "Not given"
                p_r_t = "Not given"
            ...
            if first_product is True:
                wb = Workbook()
                ws = wb.active
                item_heading = ["SKU","Image", "product title","parts_from_title", "price", "location", "quantity"]
                ...
                wb.save("crash parts fast.xlsx")
                first_product = False

            workbook = load_workbook("crash parts fast.xlsx")
            ...
            work_sheet.cell(row=max_row + 1, column=4).value = p_r_t
            work_sheet.cell(row=max_row + 1, column=5).value = price
            work_sheet.cell(row=max_row + 1, column=6).value = location
            work_sheet.cell(row=max_row + 1, column=7).value = quantity
            ...
            workbook.save("crash parts fast.xlsx")
            driver.back()
            time.sleep(2)
